...
int ge(double *A, int SIZE) {
    ...
    for (k = 0; k < SIZE; k++) {
        #pragma omp parallel for private(i, j, multiplier, mm_multiplier, tmp0, tmp1, tmp2, tmp3, tmp4, tmp5, tmp6, tmp7) schedule(static)
        for (i = k + 1; i < SIZE; i++) {
            _mm_prefetch((const char*)&A[IDX(i, k, SIZE)], _MM_HINT_T0);
            _mm_prefetch((const char*)&A[IDX(k, k, SIZE)], _MM_HINT_T0);

...
int mm(double ** first, double ** second, double ** multiply, int SIZE)
{
  ...
  double sum = 0;
  for (i = local_size; i != 0; i--) { //rows in multiply
    for (j = local_size; j != 0; j--) { //columns in multiply
      for (k = local_size; k != 0; k--) { //columns in first and rows in second
        sum = sum + first[i-1][k-1]*second[k-1][j-1];
      } 
      multiply[i-1][j-1] = sum;
      sum = 0;
    }
  }
  return 0;
